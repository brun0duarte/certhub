...
@router.get("/expiring")
def get_expiring_certs(days: int = Query(90, ge=0), pending_only: bool = False, conn=Depends(get_db)):
...
def get_by_lifecycle(status: str = "", search: str = "", conn=Depends(get_db)):
...
@router.get("/summary")
def monitor_summary(conn=Depends(get_db)):
...
def flag_for_renewal(cert_id: int, conn=Depends(get_db)):
